...
public class TaskMapper {
  ...
  public TaskDto mapTaskToTaskDto(final Task task) {
    return new TaskDto(
        ...
        task.getDescription(),
        task.getStatus(),
        task.getUsers().stream().map(User::getUsername).collect(Collectors.toList()),
  ...
  public Task mapTaskDtoToTask(final TaskDto taskDto) {
    return new Task(
      ...
      taskDto.getStatus(),
      taskDto.getUserNames().stream().map(userRepository::findByUsername).collect(Collectors.toList()),
